# tweaked version of a (weak) chess bot that can interact with the Chess module. it contains all of the functionalities that I want to include for now, but it is still slow. next step would be to try and move some code to a C extension for faster performance
...
PUZZLES_FILE = os.path.join(ABS_DIR_PATH, "data/puzzles_sample.csv")
...
ENDGAME_BONUS_VALUES = {WHITE: 
    ...
    BISHOP: [[0 for i in range(8)] for j in range(8)],
    KNIGHT: [[0 for i in range(8)] for j in range(8)],
    ROOK: [[0 for i in range(8)] for j in range(8)],
    QUEEN: [[0 for i in range(8)] for j in range(8)]}}
...
OPENINGS_DATABASE_PATH = os.path.join(ABS_DIR_PATH, "data/openings_rawdata")
...
class Chessbot:
    ...
    def __init__(self, board, thinking_time=BOT_THINKING_TIME):
        ...
        # preparing variables for zobrist hashing
        self.init_zobrist()
    ...
    # loading a selection of openings from several tsv files and converting them to internal move notation
    def load_openings_database(self):
        self.openings_database = defaultdict(list)
            ...
            with open(filename) as in1:
                ...
                for row in in1c:
                    ...
                    for m in uci_moves:
                        ...
                        self.openings_database[current_hash].append(move)
    ...
    def order_moves(self, movelist, start_move=None):
        ...
        for move in movelist:
            ...
            move_score_guess = 0
            moved_piece_type = self.board.board[y_from][x_from].piece
            captured_piece_type = self.board.board[y_to][x_to].piece
            ...
            if len(move) == 3:
                move_score_guess += PIECE_VALUES[my_chess.Piece(move[2]).piece]
            
            # moving into opponents pawn capture range with a piece other than a pawn is often bad
            if (y_to, x_to) in self.board.reachable[self.board.opponent]['pawn_attack']:
    ...
    def calculate_extension(self, move, ext_count):
        ...
        to_y, _ = move[1]
        moved_piece = self.board.board[from_y][from_x].piece
    ...
    def materialcount(self):
        sumown = sum([PIECE_VALUES[self.board.board[sq[0]][sq[1]].piece] for sq in self.board.piece_loc[self.board.to_move]])
        sumopp = sum([PIECE_VALUES[self.board.board[sq[0]][sq[1]].piece] for sq in self.board.piece_loc[self.board.opponent]])
    ...
    def king_to_corner_endgame(self):
        ...
        oppo_rank = self.board.kings[self.board.opponent][0]
        oppo_file = self.board.kings[self.board.opponent][1]
        ...
        own_rank = self.board.kings[self.board.to_move][0]
        own_file = self.board.kings[self.board.to_move][1]
    ...
    def opening_positioning(self):
        sumown = sum([OPENING_BONUS_VALUES[self.board.to_move][self.board.board[sq[0]][sq[1]].piece][sq[0]][sq[1]] for sq in self.board.piece_loc[self.board.to_move]])
        sumopp = sum([OPENING_BONUS_VALUES[self.board.opponent][self.board.board[sq[0]][sq[1]].piece][sq[0]][sq[1]] for sq in self.board.piece_loc[self.board.opponent]])
    ...
    def endgame_positioning(self):
        sumown = sum([ENDGAME_BONUS_VALUES[self.board.to_move][self.board.board[sq[0]][sq[1]].piece][sq[0]][sq[1]] for sq in self.board.piece_loc[self.board.to_move]])
        sumopp = sum([ENDGAME_BONUS_VALUES[self.board.opponent][self.board.board[sq[0]][sq[1]].piece][sq[0]][sq[1]] for sq in self.board.piece_loc[self.board.opponent]])
        
        endgame_bonus = (sumown - sumopp)

        return endgame_bonus * ENDGAME_BONUS_WEIGHT * self.endgame_weight

    # this function initializes the zobrist hash functionality by assigning each piece-square combination a random 64bit number, plus another 64bit number that is used if black is to move
    def init_zobrist(self):
        self.zobr = [[{WKING: None, WQUEEN: None, WPAWN: None, WBISHOP: None, WKNIGHT: None, WROOK: None, BKING: None, BQUEEN: None, BPAWN: None, BBISHOP: None, BKNIGHT: None, BROOK: None} for x in range(8)] for y in range(8)]
        self.zobr_black = random.getrandbits(64)

        for y in range(8):
            for x in range(8):
                for key in self.zobr[y][x]:
                    self.zobr[y][x][key] = random.getrandbits(64)

    # this funciton creates a 64bit zobrist hash to represent the current state of the board. this is done by XORing every random number that gets a hit in the current configuration (e.g. if there is a black knight on e4, then the hash will be XORed with the black knight + e4 number). the hash is currently generated from scratch every time, which is slower than updating it after each move would be. but because of the special moves castles and en passant, this implementation would be tricky and most likely I would have to change the chess class for it or even implement the hash functionality in the chess class itself instead of in the bot class, so lets ignore this for now
    def hash_zobrist(self):
        ...
        for y in range(8):
            for x in range(8):
                curr_piece = self.board.board[y][x].content
                if curr_piece != NO_PIECE:
                    h = h ^ self.zobr[y][x][curr_piece]
